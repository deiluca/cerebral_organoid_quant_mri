...
from scripts.utils.io_utils import get_orig_imgs
...
class SegmentationOverlayVisualizer(object):
    ...
    #     print(gt.shape, pred.shape)
        ...
        if self.keep_planes is not None:
            indices_dices = {k:v for k, v in indices_dices.items() if k in self.keep_planes}
        fig, axs = plt.subplots(len(indices_dices), nr_rows, figsize=(self.size_per_org_x*nr_rows, self.size_per_org_y*len(indices_dices)))
        plane_numbers = []
    #     print(len(indices_dices), indices_dices)
        for j, i in enumerate(sorted(list(indices_dices.keys()), reverse=True)):
            ...
            if self.dw_mri:
                ...
                axs[j, 0].imshow(img_i*-1 if self.mult_neg else img_i, cmap='Greys')
                axs[j, 1].imshow(img_i*-1 if self.mult_neg else img_i, cmap='Greys')
                axs[j, 2].imshow(img_i*-1 if self.mult_neg else img_i, cmap='Greys')
            else:
                if self.planes=='coronal':
                    ...
                    img_i = self.orig_imgs[self.org_id][i, ol[2]:ol[3], ol[0]:ol[1]]

                axs[j, 0].imshow(img_i*-1 if self.mult_neg else img_i, cmap='Greys')
                axs[j, 1].imshow(img_i*-1 if self.mult_neg else img_i, cmap='Greys')
                axs[j, 2].imshow(img_i*-1 if self.mult_neg else img_i, cmap='Greys')

            axs[j, 1].imshow(self.get_green_binary_colors(gt_i), cmap='Set2', alpha=0.5)
            axs[j, 2].imshow(self.get_green_binary_colors(pred_i), cmap='Set2', alpha=0.5)
            axs[j, 0].axis('off')
            axs[j, 1].axis('off')
            axs[j, 2].axis('off')
            if self.set_title:
