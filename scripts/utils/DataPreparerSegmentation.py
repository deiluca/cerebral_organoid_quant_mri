...
class DataPreparerSegmentation(object):
    ...
    def create_h5files_cyst_seg(self):
        ...
        for f in os.listdir(self.cyst_seg_gtdir):
            ...
            assert img.shape == gt.shape
            write_h5(os.path.join(self.cyst_seg_files_3dunet, f"{org_id}.h5"), img, gt)
    ...
    def create_loocv_splits_local_cyst_seg(self):
        ...
        orgs_local_cyst_seg = [x.replace('.npy', '') for x in os.listdir(self.cyst_seg_gtdir)]
        for org in range(1, 10):
            ...
            orgs_remaining = [x for x in orgs_local_cyst_seg if x not in orgs_test]  
            train, val = train_test_split(orgs_remaining, test_size=0.2, random_state=0)

            target_dir = opj(outdir, 'test')
            os.makedirs(target_dir)
            for test_org in orgs_test:
                shutil.copyfile(opj(self.cyst_seg_files_3dunet, test_org+".h5"), opj(target_dir, test_org+".h5"))

            target_dir = opj(outdir, 'train')
            os.makedirs(target_dir)
            for train_org in train:
                shutil.copyfile(opj(self.cyst_seg_files_3dunet, train_org+".h5"), opj(target_dir, train_org+".h5"))

            target_dir = opj(outdir, 'val')
            os.makedirs(target_dir)
            for val_org in val:

                shutil.copyfile(opj(self.cyst_seg_files_3dunet, val_org+".h5"), opj(target_dir, val_org+".h5"))
        print('done')